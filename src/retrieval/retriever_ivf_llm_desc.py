...
class HybridIVFRetriever:
    ...
    def __init__(
        ...
        valid_methods = ["combsum", "borda", "max_pooling", "rrf", 
        ...
        self.alpha = 0.5  # Weight for weighted_combsum (0.5 = equal weight)
        
        # BM25 parameters (optimized for information retrieval)
        self.bm25_k1 = bm25_k1  # Term frequency saturation parameter (1.2-2.0, default: 1.5)
        self.bm25_b = bm25_b  # Document length normalization (0-1, default: 0.75)
        self.bm25_delta = bm25_delta  # BM25+ delta parameter (lower bound, default: 0.5)        # Load components
        self._load_index()
        self._load_metadata()
    ...
    def _build_bm25_index(self):
        ...
        print(f"\n🔍 Building BM25 index on cluster summaries (variant: {self.bm25_variant})...")
    ...
    def _get_embedding_ranking(self, query_vector: np.ndarray, k: int) -> Tuple[List[int], List[float]]:
        ...
        return cluster_ids, distances, all_cluster_ids
    ...
    def _fusion_max_pooling(
        ...
        """Max pooling fusion: take maximum normalized score"""
        # Normalize scores
        emb_scores_norm = self._normalize_scores(emb_scores)
        bm25_scores_norm = self._normalize_scores(bm25_scores)
        ...
        for cluster_id, score in zip(emb_clusters, emb_scores_norm):
            combined_scores[cluster_id] = max(combined_scores.get(cluster_id, 0.0), score)
    ...
    def _fusion_weighted_combsum(
        ...
        """Weighted CombSum fusion: alpha * embedding + (1-alpha) * BM25"""
        # Normalize scores
        emb_scores_norm = self._normalize_scores(emb_scores)
        bm25_scores_norm = self._normalize_scores(bm25_scores)
        ...
        for cluster_id, score in zip(emb_clusters, emb_scores_norm):
            combined_scores[cluster_id] = combined_scores.get(cluster_id, 0.0) + self.alpha * score
        
        for cluster_id, score in zip(bm25_clusters, bm25_scores_norm):
            combined_scores[cluster_id] = combined_scores.get(cluster_id, 0.0) + (1 - self.alpha) * score
    ...
    def _fusion_combmnz(
        ...
        """CombMNZ fusion: Sum of scores * number of non-zero scores"""
        # Normalize scores
        emb_scores_norm = self._normalize_scores(emb_scores)
        bm25_scores_norm = self._normalize_scores(bm25_scores)
        ...
        # Track scores and count of non-zero scores for each cluster
        cluster_scores = {}
        cluster_counts = {}
        ...
        # Multiply sum by count (CombMNZ formula)
        combined_scores = {
            cluster_id: cluster_scores[cluster_id] * cluster_counts[cluster_id]
    ...
    def _fusion_zscore_combsum(
        ...
        """CombSum fusion with Z-score normalization instead of min-max"""
        # Normalize scores using Z-score
        emb_scores_norm = self._normalize_scores_zscore(emb_scores)
        bm25_scores_norm = self._normalize_scores_zscore(bm25_scores)
        
        # For embedding distance, lower is better -> invert
        emb_scores_norm = [-s for s in emb_scores_norm]
    ...
    def fuse_rankings(
        ...
        if self.fusion_method == "combsum":
            ...
            return self._fusion_max_pooling(emb_clusters, emb_scores, bm25_clusters, bm25_scores)
        elif self.fusion_method == "rrf":
            return self._fusion_rrf(emb_clusters, emb_scores, bm25_clusters, bm25_scores)
        elif self.fusion_method == "weighted_combsum":
            return self._fusion_weighted_combsum(emb_clusters, emb_scores, bm25_clusters, bm25_scores)
        elif self.fusion_method == "combmnz":
            return self._fusion_combmnz(emb_clusters, emb_scores, bm25_clusters, bm25_scores)
        elif self.fusion_method == "zscore_combsum":
            return self._fusion_zscore_combsum(emb_clusters, emb_scores, bm25_clusters, bm25_scores)
        else:
    ...
    def search(self, filename: str, k_clusters: int = 10, verbose: bool = True) -> Dict:
        ...
        emb_clusters, emb_distances, all_emb_clusters = self._get_embedding_ranking(query_vector, k_clusters)
        ...
        # Find position of real cluster in each ranking (search in full lists)
        ...
        hybrid_position = fused_clusters.index(real_cluster) + 1 if real_cluster in fused_clusters else None
...
def main():
    ...
    parser.add_argument("--bm25-delta", type=float, default=0.5,
                       help="BM25+ and BM25L delta parameter (lower bound, default: 0.5)")
